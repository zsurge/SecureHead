...
union __ENC_KEY
{
	...
	unsigned char encway;							
	unsigned char fix[16];						
...
extern union __ENC_KEY ENC_KEY;				
extern unsigned char CryptoKey[16];				
extern unsigned char random[8];					
extern unsigned char macdata[8];					

extern volatile unsigned char WriteFlag;			
extern volatile unsigned char GetNextFlag;		

extern volatile unsigned char macstate;			
extern volatile unsigned char encryptdatastatus;	
...
unsigned char ReadENCKEY(void);
void WriteENCKEY(void);
unsigned char AsciiToHex(unsigned char * pAscii, unsigned char * pHex, int nLen);
unsigned char GetNextKSN(void);
...
const char APPVERSION[] = "VB0ASTMG170320001";




volatile unsigned char WriteFlag = 0;				
volatile unsigned char GetNextFlag = 0;			

volatile unsigned char macstate = 0x00;			
volatile unsigned char encryptdatastatus = 0x00;	
...
volatile unsigned char RxdStatus;			 		
volatile unsigned char SynHead;				 	
volatile unsigned char RxdFrameStatus;			
volatile unsigned char NowLen;				 	
volatile unsigned char RxdTotalLen;			 	




volatile unsigned char SetKeyLen=0;			    
volatile unsigned char SetKeyFlag=0;				
volatile unsigned char Bcc;						
...
void NVIC_Clear_all(void)
{
	unsigned int i;
	for(i=0; i<3; i++)
		((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->ICER[i] = 0;
	for(i=0; i<3; i++)
		((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->ICPR[i] = 0;
}
...
__asm void BOOT_jump(unsigned int sp, unsigned int pc)
{
...
void BOOT_boot(void)
{
	...
	((SCB_Type *) ((0xE000E000UL) + 0x0D00UL) )->VTOR = (unsigned int) 0x00;

	((SCB_Type *) ((0xE000E000UL) + 0x0D00UL) )->AIRCR = 0x05FA0004;

	 
	BOOT_jump(*((unsigned int *) 0x00),*((unsigned int *) 0x00 + 1));
}
...
void WriteENCKEY(void)
{
	...
	if(WriteFlag==1)
	{
		...
		for(i=0;i<62;i++)
		{
 		...
 		for(i=0;i<8;i++)
 			tri_des(tempdata+i*8,(unsigned char *)SHID,(unsigned char *)SHID+8,0);	   
		__disable_irq();
		EarseBlockFlash(0x800E000);
		WriteBlockFlash(0x800E000 ,tempdata, 64);
		__enable_irq();
		return ;
	}
	else
...
unsigned char ReadENCKEY(void)
{
	...
	unsigned char tempdata[64],i,lrc,bcc;
	memset(tempdata ,0, 64);
	ReadBlockFlash(0x800E000 ,tempdata, 64);     			
	for(i=0;i<8;i++)
		tri_des(tempdata+i*8,(unsigned char *)SHID,(unsigned char *)SHID+8,1);	   
...
void send_frame(unsigned char frame_type)
{
    unsigned char i, j, bcc;
	unsigned char TxdBuf[100];
	unsigned char TempBuf[20];
	unsigned char pBKCV[6];
	unsigned char bpSEncod[10]={0xFF,0x00,0x06,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
    for (i = 0; i < 5; i++) 
	{
        TxdBuf[i] = 0x01;    
    }
    ...
    switch (frame_type) 
	{
		...
		case 0x15:
			...
			break;
		case 0x7E:					
			if(((unsigned char *)TempTrackDirtData . Value)[2] == 0x7E) 
			{
				i = 0;
				TxdBuf[i++] = 0x06; 			
				TxdBuf[i++] = ((unsigned char *)TempTrackDirtData . Value)[0]; 		
				bcc = ((unsigned char *)TempTrackDirtData . Value)[0];
				TxdBuf[i++] = 0x7E;
	            bcc ^= 0x7E;
				TxdBuf[i++] = 0x01;
	            bcc ^= 0x01;
			...
			break;
		case 0x74:
			if(((unsigned char *)TempTrackDirtData . Value)[2] == 0x74) 
			{
				i = 0;
				TxdBuf[i++] = 0x06; 			
				TxdBuf[i++] = ((unsigned char *)TempTrackDirtData . Value)[0]; 		
			...
			break;			
		case 0x22:
			if(((unsigned char *)TempTrackDirtData . Value)[2] == 0x22) 
			{
				i = 0;
				TxdBuf[i++] = 0xFF; 			
				TxdBuf[i++] = 0x06; 			
			...
			break;
		case 0x51:
			if(((unsigned char *)TempTrackDirtData . Value)[2] == 0x51) 
			{
				i = 0;
				TxdBuf[i++] = 0x06; 			
				TxdBuf[i++] = ((unsigned char *)TempTrackDirtData . Value)[0]; 		
				bcc = ((unsigned char *)TempTrackDirtData . Value)[0];
				TxdBuf[i++] = ((unsigned char *)TempTrackDirtData . Value)[2];		
				bcc ^= ((unsigned char *)TempTrackDirtData . Value)[2];
				TxdBuf[i++] = 0x0B;				
				bcc ^= 0x0B;
				TxdBuf[i++] = 0x0A;				
				bcc ^= 0x0A;
			...
			break;
		case 0x4E:							
			if(((unsigned char *)TempTrackDirtData . Value)[2] == 0x4E) 
			{
				i = 0;
				TxdBuf[i++] = 0x06; 			
				TxdBuf[i++] = ((unsigned char *)TempTrackDirtData . Value)[0]; 		
			...
			break;
		case 0x54:							
			if(((unsigned char *)TempTrackDirtData . Value)[2] == 0x54) 
			{
				i = 0;
				TxdBuf[i++] = 0x06; 			
				TxdBuf[i++] = ((unsigned char *)TempTrackDirtData . Value)[0]; 		
		...
		case 0x0A:
			{
				...
				if(1 == SetKeyFlag)
				{
					...
					GetKeyCkeckValue(ENC_KEY.temp.ksn,pBKCV,8,0);
				...
				{
					memcpy(TxdBuf,"\x06\x02\x46\x46\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x0D\x0A\x03\x00",23);
					GetKeyCkeckValue(ENC_KEY.temp.key,pBKCV,16,0);
					HexToAscii(pBKCV, bpSEncod+3, 3);
					for(bcc = 0, i = 0; i < 9; i++)
						bcc ^= bpSEncod[i];
					bpSEncod[i] = bcc;
		...
		{  
			case 0:
				...
				if (0x01 == i) 
				{
					...
					if (SynHead == 3) 
					{
						((unsigned char *)TempTrackDirtData . Value)[0] = i;
						Bcc = i;
						NowLen = 1;
						RxdStatus = 20;
					}
			...
			case 20:
				if (0x13 == i) 
				{ 
					((unsigned char *)TempTrackDirtData . Value)[NowLen++] = i;
					Bcc +=i;
					RxdStatus = 23;
					RxdTotalLen = 3;
				}
...
void DealRxData(void)
{
	...
	if (0x55 == RxdFrameStatus)
	{
		
		IWDG_ReloadCounter();
		if (((unsigned char *)TempTrackDirtData . Value)[0] == 0x01)									
		{
			for (i = 0; i < RxdTotalLen - 1; i++)				
			{
				bcc += ((unsigned char *)TempTrackDirtData . Value)[i];
			}
			if (bcc == ((unsigned char *)TempTrackDirtData . Value)[RxdTotalLen -1])
			{
				if (0x13 == ((unsigned char *)TempTrackDirtData . Value)[1])
				{
					switch (((unsigned char *)TempTrackDirtData . Value)[2])
					{
						case 0xBE:						 
								send_frame(0xBE);
								init_serial_boot();
		...
		else if(((unsigned char *)TempTrackDirtData . Value)[0] == 0x02)									
		{
			for (i = 0; i < RxdTotalLen - 1; i++)			 	   
			{
					bcc ^= ((unsigned char *)TempTrackDirtData . Value)[i];
			}
			if (bcc == ((unsigned char *)TempTrackDirtData . Value)[RxdTotalLen -1] || (bcc^0x02) == ((unsigned char *)TempTrackDirtData . Value)[RxdTotalLen -1] )
			{
				if (0x38 == ((unsigned char *)TempTrackDirtData . Value)[1])							
				{
					SetKeyFlag = 0;
				...
				else if (0x46 == ((unsigned char *)TempTrackDirtData . Value)[1] && 0x46 == ((unsigned char *)TempTrackDirtData . Value)[2])	
				{
					memset(((unsigned char *)TempTrackData . Value),0,sizeof(((unsigned char *)TempTrackData . Value)));
					SetKeyLen = from64tobits(((unsigned char *)TempTrackData . Value),&((unsigned char *)TempTrackDirtData . Value)[3],RxdTotalLen-7);
					if(((unsigned char *)TempTrackData . Value)[0] == 0xFF)
					{
						bcc = 0;	
						for(i = 0; i < SetKeyLen-1; i++)
							bcc ^= ((unsigned char *)TempTrackData . Value)[i];
						if (bcc == ((unsigned char *)TempTrackData . Value)[SetKeyLen -1])			   	
						{
							SetKeyLen =  ((unsigned char *)TempTrackData . Value)[2];					
							switch (((unsigned char *)TempTrackData . Value)[1])
							{
								case 0x13:						
									if(0x01 == SetKeyLen && 0x02 == ((unsigned char *)TempTrackData . Value)[3])
									{
										send_frame(0x13);
										SetKeyFlag = 1;
									}
								...
								case 0x0A:						
									if(SetKeyFlag > 0)
									{
										if(0x11 == SetKeyLen && 0x32==((unsigned char *)TempTrackData . Value)[3])			 
										{
											AsciiToHex(&((unsigned char *)TempTrackData . Value)[4],ENC_KEY.temp.ksn,SetKeyLen-1);
											if(GetNextKSN())
											{
												send_frame(0x0A);
												SetKeyFlag = 2;
										...
										else if(0x21 == SetKeyLen && 0x33==((unsigned char *)TempTrackData . Value)[3])	    
										{
											AsciiToHex(&((unsigned char *)TempTrackData . Value)[4],ENC_KEY.temp.key,SetKeyLen-1);
				...
				else if (0x52 == ((unsigned char *)TempTrackDirtData . Value)[1])					
				{
					switch (((unsigned char *)TempTrackDirtData . Value)[2])
					{
						case 0x22:						    
				...
				else if (0x53 == ((unsigned char *)TempTrackDirtData . Value)[1])				
				{
					switch (((unsigned char *)TempTrackDirtData . Value)[2])
					{
						case 0x18:						
							ENC_KEY.temp.status = 0;			
							ENC_KEY.temp.level = 0x31;			
							ENC_KEY.temp.encway = 0x00;			
							macstate = 0x00;					
							WriteFlag = 1;
							WriteENCKEY();						
							WriteFlag = 0;							
						...
						case 0x1D:						
	                		...
	                		break;
						case 0x58:						
							if(((unsigned char *)TempTrackDirtData . Value)[3]==0x01)
							{	
								if(0x30==((unsigned char *)TempTrackDirtData . Value)[4]|| 0x31==((unsigned char *)TempTrackDirtData . Value)[4])
								{
									ENC_KEY.temp.encmode = ((unsigned char *)TempTrackDirtData . Value)[4];	
									if(0x30 == ((unsigned char *)TempTrackDirtData . Value)[4])
						...
						case 0x74:						
							...
							if(memcmp(random,&((unsigned char *)TempTrackDirtData . Value)[4],((unsigned char *)TempTrackDirtData . Value)[3]) == 0)
							{
								macstate = 0x31;				
						...
						case 0x4C:							
							if(((unsigned char *)TempTrackDirtData . Value)[3]==0x01)
							{	
								if(0x30 == ((unsigned char *)TempTrackDirtData . Value)[4] || 0x31 == ((unsigned char *)TempTrackDirtData . Value)[4] || 0x32 == ((unsigned char *)TempTrackDirtData . Value)[4])
								{
									ENC_KEY.temp.encway = ((unsigned char *)TempTrackDirtData . Value)[4];
						...
						case 0x84:					
							if(((unsigned char *)TempTrackDirtData . Value)[3]==0x01)
							{	
								if(0x0F == ((unsigned char *)TempTrackDirtData . Value)[4])		    
								{
									send_frame(0x06);
								}
							}
	                		...
	                		break;
						case 0x85:					 
							if(((unsigned char *)TempTrackDirtData . Value)[3])
							{	
								if(0x31 == ((unsigned char *)TempTrackDirtData . Value)[4]);			 
									send_frame(0x06);
							}
							else
							{
						...
						case 0x36:						
	                		...
	                		break;
						case 0x76:						
							if(((unsigned char *)TempTrackDirtData . Value)[3]==0x10 && 0x31 == macstate)
							{
								memcpy(ENC_KEY.temp.fix,&((unsigned char *)TempTrackDirtData . Value)[4],10);
								ENC_KEY.temp.level = 0x32;
								WriteFlag = 1;
								WriteENCKEY();					
								WriteFlag = 0; 								
	                		...
	                		break;
						case 0x54:							
							if(((unsigned char *)TempTrackDirtData . Value)[3]==0x08)
							{
								memcpy(ENC_KEY.temp.sid,&((unsigned char *)TempTrackDirtData . Value)[4],8);
								WriteFlag = 1;
								WriteENCKEY();					
								WriteFlag = 0; 								
								send_frame(0x06);
						...
						case 0x4E:							
							if(((unsigned char *)TempTrackDirtData . Value)[3]==0x08)
							{
								memcpy(ENC_KEY.temp.sn,&((unsigned char *)TempTrackDirtData . Value)[4],8);
								WriteFlag = 1;
								WriteENCKEY();					
								WriteFlag = 0; 								
								send_frame(0x06);
						...
						case 0x1A:						
							if(((unsigned char *)TempTrackDirtData . Value)[3]==0x01)
							{
								ENC_KEY.temp.level = ((unsigned char *)TempTrackDirtData . Value)[4];
								WriteFlag = 1;
								WriteENCKEY();					
								WriteFlag = 0; 								
								send_frame(0x06);
