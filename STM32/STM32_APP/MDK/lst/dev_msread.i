...
#line 5 "..\\dev\\dev_msread.c"

#line 12 "..\\dev\\dev_msread.c"

#line 19 "..\\dev\\dev_msread.c"


SHA1Context foo;
AES_KEY key;
unsigned char sha[60];
...
void MSR_SendData(void)
{
	unsigned char *ch = (unsigned char *)TrackBitFlow.Value, ivec[16];
	uint16_t len = 0x10,j;
	unsigned char lrc,bcc,block1,block2,block3;
		
	if(ENC_KEY.temp.level < 0x30 && ENC_KEY.temp.level > 0x34 )	
	{
		goto ERRORDATA;											
	}
	if (1 == ENC_KEY.temp.status)								
	{
		encryptdatastatus = 0xBF;								
				
		 
		if(0x32 == ENC_KEY.temp.encway)							
		{
			block1 = ceil((Track1Data . Length-1)/16.0);
			block2 = ceil((Track2Data . Length-1)/16.0);
			block3 = ceil((Track3Data . Length-1)/16.0);
			len = block1*16 + block2*16 + block3*16 + 84 ;		
		}
		else													
		{
			block1 = ceil((Track1Data . Length-1)/8.0);
			block2 = ceil((Track2Data . Length-1)/8.0);
			block3 = ceil((Track3Data . Length-1)/8.0);
			len = block1*8 + block2*8 + block3*8 + 84 ;  		
		}
		
		if(!block1 && !block2 && !block3)						
			goto ERRORDATA;
		if(!Track1Data . Length)
		{												   	
			encryptdatastatus &= 0xF6;							
			len -= 20;
		}
		else
		{
			SHA1Init (&foo);
			SHA1Update (&foo, &((unsigned char *)Track1Data . Value)[1], Track1Data . Length-1);			
			SHA1Final (&foo, sha);
		}
		if(!Track2Data . Length)
		{
			encryptdatastatus &= 0xED;							
			len -= 20;
		}
		else
		{
			SHA1Init (&foo);
			SHA1Update (&foo, &((unsigned char *)Track2Data . Value)[1], Track2Data . Length-1);			
			SHA1Final (&foo, sha+20);
		}
		if(!Track3Data . Length)
		{
		 	encryptdatastatus &= 0xDB;							
			len -= 20;
		}
		else
		{
			SHA1Init (&foo);
			SHA1Update (&foo, &((unsigned char *)Track3Data . Value)[1], Track3Data . Length-1);			
			SHA1Final (&foo, sha+40);
		}		 
								
		memcpy(ch,"\xFF\x02\x00\x00\x80\x3F\x00\x00\x00\x00",10);
		ch[2] = (unsigned char)(len-7);								 
		ch[3] = ((len-7)>>8);
		if(Track1Data . Length)ch[6] = Track1Data . Length-1;
		if(!Track1Data . Length)ch[5] = 0x37;
		if(Track2Data . Length)ch[7] = Track2Data . Length-1;
		if(Track3Data . Length)ch[8] = Track3Data . Length-1;
		ch[10] = encryptdatastatus;								
		
		if (0x31 == ENC_KEY.temp.encmode)				   		
		{
			CalcCryptoKey();									
		}
		else if (0x30 == ENC_KEY.temp.encmode)			 		
		{
			memcpy(CryptoKey,ENC_KEY.temp.fix,16);				
		}
		if( 0x30 == ENC_KEY.temp.encway)						
		{
			goto ERRORDATA;
		}
		else if(0x31 == ENC_KEY.temp.encway)					
		{
			tri_des(((unsigned char *)Track1Data . Value)+1,CryptoKey,CryptoKey+8,0);
			for(j = 1;j< block1;j++)							
			{
				xor(((unsigned char *)Track1Data . Value)+1+j*8,((unsigned char *)Track1Data . Value)+j*8-7,8);
				tri_des(((unsigned char *)Track1Data . Value)+1+j*8,CryptoKey,CryptoKey+8,0);
			}
			memcpy(ch+11,((unsigned char *)Track1Data . Value)+1,block1*8);

			tri_des(((unsigned char *)Track2Data . Value)+1,CryptoKey,CryptoKey+8,0);
			for(j = 1;j< block2;j++)							
			{
				xor(((unsigned char *)Track2Data . Value)+1+j*8,((unsigned char *)Track2Data . Value)+j*8-7,8);
				tri_des(((unsigned char *)Track2Data . Value)+1+j*8,CryptoKey,CryptoKey+8,0);
			}
			memcpy(ch+11+block1*8,((unsigned char *)Track2Data . Value)+1,block2*8);
			
			tri_des(((unsigned char *)Track3Data . Value)+1,CryptoKey,CryptoKey+8,0);
			for(j = 1;j< block3;j++)							
			{
				xor(((unsigned char *)Track3Data . Value)+1+j*8,((unsigned char *)Track3Data . Value)+j*8-7,8);
				tri_des(((unsigned char *)Track3Data . Value)+1+j*8,CryptoKey,CryptoKey+8,0);
			}
			memcpy(ch+11+block1*8+block2*8,((unsigned char *)Track3Data . Value)+1,block3*8);
		}
		else if(0x32 == ENC_KEY.temp.encway)					
		{
			...
			if(block1)
			{
				...
				memcpy(ch+11,((unsigned char *)Track1Data . Value)+1,block1*16);
			...
			{
				memset(ivec,0,sizeof(ivec));
				memcpy(ch+11+block1*16,((unsigned char *)Track2Data . Value)+1,block2*16);
			...
			{
				memset(ivec,0,sizeof(ivec));
				memcpy(ch+11+(block1+block2)*16,((unsigned char *)Track3Data . Value)+1,block3*16);
		...
		if(block3)
		{
			memcpy(ch+len-33,sha+40,20);						
			if(block2)
			{
				memcpy(ch+len-53,sha+20,20);					
				if (block1)
					memcpy(ch+len-73,sha,20);   				
			}
			else if (block1)
				memcpy(ch+len-53,sha,20);   					
		}		 
		else
		{
			if(block2)
			{
				memcpy(ch+len-33,sha+20,20);					
				if (block1)
					memcpy(ch+len-53,sha,20);   				
			}
			else if (block1)
				memcpy(ch+len-33,sha,20);   					
		}
				
		if((encryptdatastatus & 0x80) == 0x80)
			memcpy(ch+len-13,ENC_KEY.temp.ksn,10);				

		for(j = 4,lrc = 0,bcc = 0;j<len-3;j++)					
		{
			lrc ^= ch[j];
			bcc += ch[j];
		}
	...
	else if(0 == ENC_KEY.temp.status)			  					
	{
		len = 1+Track3Data . Length+Track2Data . Length+Track1Data . Length;
		ch[0]=0xFF;
		if(Track3Data . Length)((unsigned char *)Track3Data . Value)[Track3Data . Length-1] = 0x0D;
		if(Track2Data . Length)((unsigned char *)Track2Data . Value)[Track2Data . Length-1] = 0x0D;
		if(Track1Data . Length)((unsigned char *)Track1Data . Value)[Track1Data . Length-1] = 0x0D;
		memcpy(ch+1,((unsigned char *)Track3Data . Value),Track3Data . Length);
		memcpy(ch+1+Track3Data . Length,((unsigned char *)Track2Data . Value),Track2Data . Length);
		memcpy(ch+1+Track3Data . Length+Track2Data . Length,((unsigned char *)Track1Data . Value),Track1Data . Length);
	}
	else
	{
ERRORDATA:
