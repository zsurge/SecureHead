...
#line 119 "D:\\Keil\\ARM\\ARMCC\\bin\\..\\include\\stdarg.h"
...
int dev_com_open(void)
{
	...
	USART_DeInit(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)));
...
int dev_com_read(unsigned char *buf, int len)
{
	...
	while (rec_head0 != rec_end0) {
		*buf = ((uint8_t *)Track1TimeFlow . Value)[rec_head0++];
		if(rec_head0 >= 512)
			rec_head0 = 0;
		buf++;
		i++;
...
int dev_com_write(unsigned char *buf, int len)
{
	...
	while (len != 0) {
		...
		USART_SendData(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)), buf[i]);
		i++;
		len--;
	}
	return i;
...
void uart_printf(char *fmt,...)
{
    ...
    dev_com_write((unsigned char *)string, length);  
    __va_end(ap);
}
