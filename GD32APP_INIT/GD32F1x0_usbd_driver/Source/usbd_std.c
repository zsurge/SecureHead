...
#ifdef LPM_ENABLED
static uint8_t* usbd_bos_descriptor_get (usbd_core_handle_struct *pudev, uint8_t index, uint16_t *pLen);
#endif
...
//    DBG_S("1.usbd_out_transaction EP_NUM = %d\r\n",ep_num);
...
//                 DBG_S("2.usbd_out_transaction ep->trs_len = %d\r\n",ep->trs_len);
                /* continue to receive remain data */
//                DBG_H("usbd_out_transaction:",(char *)ep->trs_buf, ep->trs_len);
                usbd_ep_rx(pudev, EP0_OUT, ep->trs_buf, (uint16_t)ep->trs_len);
            } else {
                if (USBD_CONFIGURED == pudev->status) {
                    /* device class handle */
//                     DBG_S("3.usbd_out_transaction USBD_RX\r\n");
        ...
        if (USBD_CONFIGURED == pudev->status) {
//            DBG_S("4.usbd_out_transaction USBD_RX\r\n");
