...
/*!
    ...
    \retval     none
*/
void  usbd_ep_init (usbd_core_handle_struct *pudev, const void *ep_desc)
{
    usb_ep_struct *ep;
...
void  usbd_ep_rx (usbd_core_handle_struct *pudev, uint8_t ep_addr, uint8_t *pbuf, uint16_t buf_len)
{
    ...
    if (buf_len > 62U) {
        buf_len = (buf_len + 31U) & ~31U;
        (pbuf_reg + ep_num)->rx_count = ((buf_len << 5) - 1U) | 0x8000U;
    } else {
        buf_len = (buf_len + 1) & ~1;
        (pbuf_reg + ep_num)->rx_count = buf_len << 9;
    }

    EP1_OUT_Callback(ep->trs_buf);
