...
/*!
    ...
    \param[out] none
    \retval     noneTAMPER_KEY_EXTI_IRQn
*/
...
{
    uint32_t temp_priority = 0x00, temp_pre = 0x00, temp_sub = 0x00;
    ...
    temp_priority = (uint32_t)nvic_irq_pre_priority << (0x4 - temp_pre);
    temp_priority |= nvic_irq_sub_priority &(0x0F >> (0x4 - temp_sub));
    temp_priority = temp_priority << 0x04;
    NVIC->IP[nvic_irq] = temp_priority;
