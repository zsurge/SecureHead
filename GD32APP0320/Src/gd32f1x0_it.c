...
void EXTI0_1_IRQHandler(void)											/*刷卡中断响应函数*/
{
	...
	set_tick(ENDTIMEOUT);
	MSR_WaitForReady();
//	if ((RESET != exti_interrupt_flag_get(EXTI_0)) && ((ENC_KEY.temp.selecttrack == 0x30) || (getbit(1,ENC_KEY.temp.selecttrack) == 0x02))) {
		if ((RESET != exti_interrupt_flag_get(EXTI_0))) {
		/* clear channel 2 interrupt bit */
...
//	if (RESET != exti_interrupt_flag_get(EXTI_1) && ((ENC_KEY.temp.selecttrack == 0x30) || (getbit(2,ENC_KEY.temp.selecttrack) == 0x04))) {
...
void EXTI4_15_IRQHandler(void)											/*刷卡中断响应函数*/
{
	set_tick(ENDTIMEOUT);
	MSR_WaitForReady();
	//if (RESET != exti_interrupt_flag_get(EXTI_7) && ((ENC_KEY.temp.selecttrack == 0x30) || (getbit(0,ENC_KEY.temp.selecttrack) == 0x01))) {
