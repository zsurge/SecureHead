...
uint8_t DecodeTrackData(uint8_t type)
{
	...
	uint8_t BitCnt = 0,SyncStep=0,bFlag=0,bData=0;
...
//			    if((ENC_KEY.temp.selecttrack == 0x30) || (getbit(0,ENC_KEY.temp.selecttrack) == 0x01))
			    {
    			    Track1TimeFlow.Value[Track1TimeFlow.Length++] = TmrCnt; 
    			}
    			break;
			case 1:
//				if((ENC_KEY.temp.selecttrack == 0x30) || (getbit(1,ENC_KEY.temp.selecttrack) == 0x02))
                {
    			    Track2TimeFlow.Value[Track2TimeFlow.Length++] = TmrCnt; 
    			}
    			break;
			case 2:
//			    if((ENC_KEY.temp.selecttrack == 0x30) || (getbit(2,ENC_KEY.temp.selecttrack) == 0x04))
                {
    			    Track3TimeFlow.Value[Track3TimeFlow.Length++] = TmrCnt; 
    			}
    			break;
		}
	//}
